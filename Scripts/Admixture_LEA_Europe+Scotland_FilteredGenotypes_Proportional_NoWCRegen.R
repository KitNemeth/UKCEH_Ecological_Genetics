...
vcf2lfmm("Europe+Scotland_FilteredGenotypes_Proportional_NoWCRegen.vcf", output.file = "Europe+Scotland_FilteredGenotypes_Proportional_NoWCRegen", force = TRUE)

# Perform Principal Component Analysis (PCA) on LFMM data
pc = pca("Europe+Scotland_FilteredGenotypes_Proportional_NoWCRegen.lfmm", scale = TRUE)
...
project = snmf("Europe+Scotland_FilteredGenotypes_Proportional_NoWCRegen.geno",
               K = 1:12,
               entropy = TRUE,
               repetitions = 10,
               project = "new")
...
out = "cross-entropy criterion_Europe+Scotland_FilteredGenotypes_Proportional_NoWCRegen.svg"
svg(out)
...
Populations_data <- read.table("geno_poplist_Europe+Scotland_FilteredGenotypes_Proportional_NoWCRegen.txt", header = TRUE, sep = "\t")

# Read the desired order from a file
desired_order <- read.csv("desired_order_Europe+Scotland_FilteredGenotypes_Proportional_NoWCRegen.txt", header = FALSE)[, 1]
...
out = "Admixture_Europe+Scotland_FilteredGenotypes_Proportional_NoWCRegen.svg"
svg(out, width = 20, height = 10)

# Set plot parameters
par(oma = c(4.5, 1, 1, 1), mar = c(7, 4, 0, 7), mgp = c(2, 1, 0), xpd = TRUE)

# Create the barplot
barplot(t(as.matrix(tbl[, 3:8])), col = col, xaxt = "n", border = NA, xlim = c(0, nrow(tbl)), width = 1, space = spaces, names.arg = rep("", nrow(tbl)), ylab = "Admixture coefficients", cex.lab = 2)

# Add x-axis labels with line breaks
axis(1, line = -0.2, at = breaks[1:(length(breaks) - 1)] + ((0:(length(breaks) - 2)) * 2) + ((breaks[2:length(breaks)] - breaks[1:(length(breaks) - 1)]) / 2), lwd = 0, lwd.ticks = 1, labels = gsub("_", " ", tbl$pop[breaks]), las = 2, cex.axis = 1.5)
